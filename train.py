...
def collect_stats(model, data_loader, num_batches):
    ...
    for i, (image, targets, paths, _) in tqdm(enumerate(data_loader), total=num_batches):
        image = image.to('cpu', non_blocking=True).float() / 255
        model(image)
        if i >= num_batches:
            break
...
def callibrate(model, data_loader):
...
def train(hyp, opt, device, callbacks):
    ...
    print("--------------- intiial learning rate  is: ", )
...
### Evaluate before callibration
    print("----------- EVAL before calib")
    if RANK in {-1, 0}:
    ...
    # calibrate
    print("___________________ CALIB")
    torch.use_deterministic_algorithms(False)
    callibrate(model.cpu(), train_loader)

### Evaluate after callibration
    print("----------- EVAL after calib")
    if RANK in {-1, 0}:
        #LOGGER.info(f"\n{epoch - start_epoch + 1} epochs completed in {(time.time() - t0) / 3600:.3f} hours.")
        LOGGER.info(f"\nValidating {f}...")
        results, _, _ = validate.run(
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            print("\n ___________________________ Image tesnor shape: ", imgs.shape)
            # Warmup
            if ni <= nw:
                xi = [0, nw]  # x interp
            ...
            print("\n ___________________ forward pass")
            # Forward
    ...
    torch.onnx.export(model, dummy_input, "nd_yolov5s_qat.onnx", verbose=True, opset_version=13, enable_onnx_checker=False)
...
def parse_opt(known=False):
    ...
    parser.add_argument("--weights", type=str, default="/home/ubuntu/mahesh/yolov5/tycho/v640_yolo_weights.pt", help="initial weights path")
    parser.add_argument("--cfg", type=str, default="", help="model.yaml path")
    parser.add_argument("--data", type=str, default="/home/ubuntu/mahesh/yolov5/tycho/TYCHO_USUK_pcw_fd.yaml", help="dataset.yaml path")
    parser.add_argument("--hyp", type=str, default="/home/ubuntu/mahesh/yolov5/tycho/hyp.scratch-low.yaml", help="hyperparameters path")
